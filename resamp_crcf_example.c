...
{
    // options
    float        r    = .5f;   // resampling rate (output/input)
    ...
    //read stills file in
    FILE* f = fopen("Stills8k.txt", "r");
    int j = 0;
    int numbersx = 8000;
    float complex numbers[numbersx];
    double num = 0;
    float complex tempNum = 0;
    char * line = NULL;
    size_t len = 0;
    ssize_t read = 0;
    char *ptr;
    ...
    //printf("got here 1");
    //printf("%lu", sizeof(numbers) / sizeof(double) );

    while ((j<8000)&&((read = getline(&line, &len, f)) != -1)) {
    ...
    printf("got here 2");
     ...
     // output buffer with extra padding for good measure
    unsigned int yNum_len = (unsigned int) ceilf(1.1 * numbersx * r) + 4;

    // arrays
    float complex yNum[yNum_len];
    ...
    resamp_crcf_execute_block(q, numbers, numbersx, yNum, &ny);
